...
namespace TheRanger
{
    public static class ImageHelper
    {
        ...
        {
            if (IsSafeExistingImage(imagePath)) return WebPath(imagePath);
        ...
        public static List<string> ExistingImages(string relativeDirectory, bool gameDrive)
        {
            ...
            foreach (string file in Directory.GetFiles(physical))
            {
                ...
                if (gameDrive)
                {
                    bool match = name.StartsWith("D", StringComparison.OrdinalIgnoreCase) || name.StartsWith("game-drive-", StringComparison.OrdinalIgnoreCase) || name.Equals("Big Five Adventure.jpg", StringComparison.OrdinalIgnoreCase) || name.Equals("Bushveld Discovery Drive.png", StringComparison.OrdinalIgnoreCase) || name.Equals("Golden Hour Safari.jpg", StringComparison.OrdinalIgnoreCase) || name.Equals("Night Safari Experience.jpg", StringComparison.OrdinalIgnoreCase) || name.Equals("Sunrise Wildlife Drive.jpg", StringComparison.OrdinalIgnoreCase);
                    if (match) result.Add("/Images/" + name);
                }
                else
                {
                    bool match = name.StartsWith("A", StringComparison.OrdinalIgnoreCase) || name.StartsWith("accommodation-", StringComparison.OrdinalIgnoreCase) || name.Equals("37-Selous-Impala-Camp.webp", StringComparison.OrdinalIgnoreCase) || name.Equals("57-Mdonya-Old-River-Camp.webp", StringComparison.OrdinalIgnoreCase) || name.Equals("Family Safari Tent.png", StringComparison.OrdinalIgnoreCase) || name.Equals("Glamping Tent.avif", StringComparison.OrdinalIgnoreCase) || name.Equals("Luxury Tent.avif", StringComparison.OrdinalIgnoreCase) || name.Equals("River Tent.jpg", StringComparison.OrdinalIgnoreCase) || name.Equals("Safari Chalet.webp", StringComparison.OrdinalIgnoreCase) || name.Equals("Handcrafted-Luxury-Safari-tents-Luxury-Tent-Manufacturer.jpg", StringComparison.OrdinalIgnoreCase) || name.Equals("romantic-tented-chalet-south-africa7.jpg", StringComparison.OrdinalIgnoreCase) || name.Equals("luxury-glamping-tent-safari-sunset-stock-photo-scaled.webp", StringComparison.OrdinalIgnoreCase);
                    if (match) result.Add("/Images/" + name);
                }
            }
        ...
        private static string WebPath(string imagePath)
        {
            string normalized = imagePath.Replace("\\", "/").Trim();
            if (normalized.StartsWith("~/", StringComparison.Ordinal)) normalized = normalized.Substring(2);
